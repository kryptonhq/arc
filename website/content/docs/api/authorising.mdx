...
```mermaid
sequenceDiagram
    Client->>Arc: connect, receive socket_id
    Client->>Your backend: POST /realtime/auth (socket_id, channel_name)
    Your backend->>Your backend: is this user allowed in this channel?
    Your backend-->>Client: { auth: "key:signature", channel_data }
    Client->>Arc: pusher:subscribe with auth
    Arc-->>Client: subscription_succeeded
```
